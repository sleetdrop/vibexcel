...
- [ ] **R2 — Candidate legality and local dead ends.** On all five pages, verify displayed digits equal the chain projection and are a subset of independent row, column, and box candidates after legal edits. Chain eliminations may legitimately narrow the basic set. Test a legal-looking wrong digit that creates a dead cell or unit without a direct duplicate, then clear it. Baseline and five legal edits passed; four valid dead-end cases passed, while Master recovery must be repeated after the fixed-clue defect is addressed. See [ROUND-02.md](ROUND-02.md).
- [ ] **F1 — Fixed-clue integrity (blocking defect).** Prevent Delete/Backspace from removing gray clue formulas while keeping green-underlined centers and the Coach selector editable. Verify all five pages in native Excel, save/reopen, and run [audit_clue_protection.py](audit_clue_protection.py) and the existing workbook suite. The regression check currently fails on production by design.
